...
in {
  ...
  # - value: the Nix attrset of *only* the keys this repo wants to own
  ...
  mkMergeActivation = {
    ...
    declaredFile = (formatsFor format).generate "${label}-declared.${format}" value;
  in ''
    merge_class_a_file() {
    ...
    merge_class_a_file "${label}" "${format}" "${declaredFile}" "${dest}" "${backupDir}"
  '';
}
