      ...
      home.file =
        {
          ...
          ".local/bin/agents-diff" = {
            ...
            text = ''
              ...
              # agent tools, what the next switch would change and which live
              # keys are app-owned (not declared in Nix, so never touched by
              # the merge). Read-only: never writes to any live file. See
              # docs/management-policy.md section 4 for the promotion workflow.
              set -uo pipefail
        ...
        // lib.optionalAttrs enableAgentSkills {
