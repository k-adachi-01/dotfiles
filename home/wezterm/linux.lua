...
local function proc_cwd(pid)
	...
	local link = "/proc/" .. tostring(pid) .. "/cwd"
	local ok, path = pcall(function()
		return wezterm.readlink(link)
	end)
	if ok and path and path ~= "" then
		return path
	end
	-- fallback: run readlink via shell
	local f = io.popen("readlink -f " .. link .. " 2>/dev/null")
	if f then
...
-- Try to get CWD from the foreground process (works even when shell is not updating OSC 7)
	...
	if ok and proc and proc.pid then
		local cwd = proc_cwd(proc.pid)
		if cwd and cwd ~= "" then
			return cwd
		end
	end
...
local function spawn_command(domain, cwd)
	...
	if cwd ~= nil and cwd ~= "" then
		spawn.cwd = cwd
		spawn.set_environment_variables = { PWD = cwd }
	end
	return spawn
end
...
local function wsl_shell_spawn_command(domain, cwd)
	...
	return {
		domain = domain,
		cwd = linux_home,
		args = { "/bin/bash", "-lc", 'cd -- "$1" && exec /bin/bash -l', "bash", cwd },
	}
end
		...
		action = wezterm.action_callback(function(window, pane)
			...
			if domain_name and domain_name:match("^WSL:") then
				spawn = {
					...
					cwd = linux_home,
					args = { "/bin/bash", "-lc", 'cd -- "$1" && codex; exec /bin/bash -l', "bash", cwd },
				}
			else
				spawn = {
