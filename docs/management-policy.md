...
| Agent Skills flake input | `github:k-adachi-01/agent-skills` + `/etc/nix/nix.custom.conf` の `access-tokens`（PR12で `git+https` から変更） | 同左（完了） | PR5 完了、PR12 で認証方式修正 |
...
### Agent Skills input の実装メモ（PR5で確定、PR12で認証方式を変更）

**PR5時点の設計（廃止）**: `git+https://github.com/k-adachi-01/agent-skills.git` を使い、`nix/home.nix` の `programs.git.settings.credential` で設定済みの `gh auth git-credential` ヘルパーを再利用する方式だった。`access-tokens` を `nix.conf` に書く（＝新しい秘密情報をディスクに増やす）ことを避けるのが動機だった。

**PR12で判明した問題**: `sudo darwin-rebuild switch` は root として flake を評価する。root の `$HOME` は `/var/root` になり、adachi 用に home-manager が生成した `~/.config/git/config` の credential ヘルパーは一切参照されない。さらに `GH_CONFIG_DIR=/Users/adachi/.config/gh` で adachi の `hosts.yml` を無理やり読ませても失敗する。理由は `gh auth login` の OAuth トークン本体が **adachi の macOS ログインキーチェーン**に保存されており（`hosts.yml` にはユーザー名しか残らない）、root プロセスは他ユーザーのログインキーチェーンを非対話的に復号できないため。つまり `git+https` + `gh auth git-credential` は adachi の対話シェルでは動くが、root からのフェッチが必要になった瞬間（`flake.lock` 更新直後の初回フェッチ等）に必ず失敗する、構造的に壊れた設計だった。

**PR12以降の設計**: input を `github:k-adachi-01/agent-skills`（Nix 組み込みの GitHub API フェッチャー）に戻し、fine-grained PAT（対象リポジトリ1つ、Contents: Read-only）を発行して `/etc/nix/nix.custom.conf` に `access-tokens = github.com=<PAT>` として1行追加する。この方式を選ぶ理由:

- `nix.enable = false`（Determinate Nix 採用のため nix-darwin は nix.conf を管理しない）なので、そもそも `nix.extraOptions` で宣言的に注入する経路がない。`/etc/nix/nix.custom.conf` は Determinate Nix インストーラが「ユーザーによるカスタマイズ用」として最初から `!include` している、まさにこの用途のためのファイル
- root と adachi のどちらから叩いても同じ Nix daemon 設定を見るため、ユーザーコンテキスト差分による非対称な失敗が原理的に起きない（git CLI・gh・Keychain を一切経由しない）
- SSH deploy key + `git+ssh://` という代替も検討したが、システム全体の SSH 設定に手を入れると adachi 自身の他の GitHub SSH 認証(push 等)に干渉するリスクがあり、影響範囲が広がるため採用しなかった
- PAT は fine-grained スコープで `k-adachi-01/agent-skills` 1リポジトリの読み取りのみに絞る。gh の OAuth トークン（`gh auth login` のデフォルトスコープ）より狭い権限になる

**セットアップ手順（このリポジトリのファイルには一切書かない、ローカル1回限りの手順）**:

1. GitHub で fine-grained PAT を発行: Settings → Developer settings → Personal access tokens → Fine-grained tokens。Repository access は `k-adachi-01/agent-skills` のみに限定し、Repository permissions は `Contents: Read-only` のみ付与
2. `sudo vim /etc/nix/nix.custom.conf` などで以下の1行を追記（既存のインストーラコメントはそのまま残す）:
   ```
   access-tokens = github.com=<発行したPAT>
   ```
3. `nix flake update agent-skills --flake ~/.config/nix-darwin && sudo darwin-rebuild switch --flake ~/.config/nix-darwin#macbook` で疎通確認

`/etc/nix/nix.custom.conf` は Nix インストーラが管理する、このリポジトリの外側にあるファイルであり、`.gitignore` の対象にすらならない（そもそも repo の外）。**このファイルの内容やPATの値をこのリポジトリの追跡ファイル・スクリプト出力・コミットメッセージに書き写さないこと。**
...
7. `/etc/nix/nix.custom.conf`（`agent-skills` 用 `access-tokens` を含む、PR12）はこのリポジトリの外側にある Nix インストーラ管理ファイル。この中身をこのリポジトリの追跡ファイルへコピーしない。将来 `nix.custom.conf` の内容をバックアップ・移行手順として文書化する場合も、実際のトークン値ではなく「発行手順」だけを書く
