...
Claude/Cursor（Nix store symlink）はアプリ側の書き込みが効かない（例: Cursor `cli-config.json` の `hasChangedDefaultModel`）。**このドキュメントの「目標の管理方式」列は将来の設計であり、上の表で完了と記載されるまでは現在の挙動と異なる。**
