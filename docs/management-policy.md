...
2. `sudo darwin-rebuild switch --flake ~/.config/nix-darwin#macbook`（ローカル `~/agent-skills` を読み直す）
3. `~/.claude/skills` `~/.cursor/skills` `~/.agents/skills`（symlink）と `~/.codex/skills` `~/.kiro/skills`（同期ディレクトリ）の反映を表示

`flake.lock` の更新は不要（path input はローカルディレクトリを直接参照するため）。
