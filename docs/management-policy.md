...
| Agent Skills flake input | `path:/Users/adachi/agent-skills`（ローカル checkout、GitHub 認証不要） | 同左（完了） | PR5 で一時的に GitHub pin したが PR13 で path に戻した |
...
**現在の設計（PR13で確定）**: `path:/Users/adachi/agent-skills` を flake input として使う。Nix はローカルディスクを読むだけなので、`darwin-rebuild switch` の評価中に GitHub 認証は一切不要。`~/agent-skills` は private repo（`k-adachi-01/agent-skills`）の git checkout であり、`skills-push` が commit/push を担当する（push 時だけ `gh auth login` 等の通常 git 認証が必要。Nix とは無関係）。

**PR5で一時的に入れた GitHub pin（廃止）**: 再現性向上のため `git+https://github.com/k-adachi-01/agent-skills.git` に切り替えたが、`sudo darwin-rebuild switch` は root として flake を評価するため `gh auth git-credential` ヘルパーが効かず、root は adachi の macOS ログインキーチェーンに保存された gh OAuth トークンを非対話的に読めない。`flake.lock` 更新のたびに GitHub ログインを求められる問題が発生した。PR12 で `github:` + PAT (`access-tokens`) を試したが、長期アクセスキー管理が必要になるため不採用。PR13 で `path:` に戻した。

**新しい Mac での bootstrap**: dotfiles clone に加え、`~/agent-skills` を clone する（`gh auth login` 済みなら `gh repo clone k-adachi-01/agent-skills ~/agent-skills`）。その後 `sudo darwin-rebuild switch` するだけで skills が有効になる。

skills を更新する日常運用は `skills-push` スクリプト（`~/.local/bin/skills-push "commit message"`）に集約した。実行内容:

1. `~/agent-skills` の変更を commit・push（GitHub へのバックアップ。Nix フェッチではない）
2. `nix flake lock --update-input agent-skills`（ローカル `~/agent-skills` の内容ハッシュを `flake.lock` に反映。GitHub 認証不要）
3. `sudo darwin-rebuild switch --flake ~/.config/nix-darwin#macbook`
4. 反映確認
5. 変更された `flake.lock` を dotfiles へ commit・push

`path:` input は GitHub からフェッチしないが、Nix は内容アドレス（`narHash`）でディレクトリを pin する。`~/agent-skills` を編集したあと `flake.lock` を更新せずに `switch` すると `NAR hash mismatch` エラーになる。手動で試す場合は switch の前に `nix flake lock --update-input agent-skills --flake ~/.config/nix-darwin` を実行する。
